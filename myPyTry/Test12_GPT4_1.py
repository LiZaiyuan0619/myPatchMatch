# Since the provided SFM script is focused on image loading and camera parameters,
# I'll draft a basic framework for dense point cloud reconstruction that includes:
# 1. Depth map generation
# 2. Mesh generation and filtering
# 3. Dense point cloud creation

# Note: This is a high-level framework and may require additional libraries and fine-tuning
# based on the specific requirements and data.
...
def generate_depth_map(image_list, K):
    # 确保至少有两幅图像
    if len(image_list) < 2:
        raise ValueError("Need at least two images to generate depth map")
    ...
    for i in range(len(image_list) - 3):
        # 选择相邻的图像对
        img_left = cv2.imread(image_list[i])
        img_right = cv2.imread(image_list[i + 1])
        ...
        # 视差图转换为深度图
        focal_length = K[0, 0]  # 假设焦距是K矩阵的(0,0)元素
        baseline = 1.0  # 假设相机间距离为1单位，需要根据实际情况调整
    ...
    # 合并所有深度图以得到最终的深度图
    # 这里我们简单地取平均，但也可以采用更复杂的方法
    depth_map = np.mean(depth_maps, axis=0)

    # # 映射深度图到伪彩色
    # depth_map_color = cv2.applyColorMap((depth_map/depth_map.max() * 255).astype(np.uint8), cv2.COLORMAP_JET)
    ...
    # 保存深度图
    cv2.imwrite('depth_map_double_2_GPT4.png', depth_map)

    return depth_map

def create_mesh_from_depth_map(depth_map):
    # This function will create a mesh from the depth map.
    # Further implementation needed based on the method chosen for mesh generation.
    pass

def filter_mesh(mesh):
    # This function will filter and refine the mesh.
    # Implementation will vary based on the filtering techniques used.
    pass

def create_dense_point_cloud(mesh):
    # This function will create a dense point cloud from the mesh.
    # It can be implemented using libraries like Open3D.
    pass

# Example usage
# images = load_images() # Load your images here
# camera_params = load_camera_parameters() # Load camera parameters here
# depth_maps = generate_depth_map(images, camera_params)
# mesh = create_mesh_from_depth_map(depth_maps)
# filtered_mesh = filter_mesh(mesh)
# dense_point_cloud = create_dense_point_cloud(filtered_mesh)
...
class Sfm():
    ...
    def __call__(self, enable_bundle_adjustment=1):
        ...
        generate_depth_map(self.img_obj.image_list, self.img_obj.K)
