...
##########################################################################################################
        ...
        # 使用 pyvista 的 read 函数从一个 PLY 文件中加载稀疏点云数据。ply 对象是一个 PolyData 类型的对象，其中包含了点云的点、法线等信息。
        # ply = pv.read('res\\myCV.ply')
        # Load sparse point cloud from PCD file
        ...
        # # 基于点云和法向量创建网格
        # mesh = create_mesh(pcd, normals, radius = 0.1)
        #
        # # 可视化网格
        # o3d.visualization.draw_geometries([mesh])

        # Compute neighbor information for each point using PyVista
        # 使用 pyvista 中的 KDTree 构建一个KD树，以便在重建过程中查找每个点的邻居。这里使用了半径搜索，找到半径为0.1的邻居点。
        # kdtree = o3d.geometry.KDTreeFlann(pcd)
        mesh = o3d.geometry.TriangleMesh()
        ...
        # 利用法线向量和邻居信息，通过创建新的 PolyData 对象来重建多边形。这里使用了邻居点的坐标以及通过 triangulate 函数生成的三角面片。
        # 这些是二维多边形的顶点，但是我们需要三维多边形，所以需要使用法线向量来指定多边形的方向。
        # polygons = []
        # for i in range(total_points.shape[0]):
        #     neighbors = idx[i]
        #     normal = normals[i]
        #     polygons.append(o3d.geometry.Polygon(pcd.points[neighbors], normal=normal))

        # Extend polygons to fill in gaps and remove small areas
        # 对重建的多边形进行扩展，以填补缺失的区域，并使用 decimate 函数降低多边形的分辨率。
        # extended_polygons = []
        ...
        # # 利用稠密点云和多边形，生成深度图。这里使用了 triangulate 函数将点云转为三角面片，并提取了深度信息
        ...
        # # 从深度图中提取多边形的点。这里使用了 find_contours 函数，提取了深度图中的等高线。
        # polygon_points = []
