...
def generate_depth_map(points_3d, intrinsic_matrix, distortion_coeffs, image_shape):
    # Create an empty depth map
    depth_map = np.zeros(image_shape, dtype=np.float32)

    # Project 3D points to 2D image coordinates
    points_2d, _ = cv2.projectPoints(points_3d, np.zeros((3, 1)), np.zeros((3, 1)), intrinsic_matrix, distortion_coeffs)

    # Iterate through the 2D points and update the depth map
    for i in range(points_2d.shape[0]):
        x, y = int(round(points_2d[i, 0, 0])), int(round(points_2d[i, 0, 1]))
        if 0 <= x < image_shape[1] and 0 <= y < image_shape[0]:
            depth_map[y, x] = points_3d[i, 2]  # Use the Z coordinate as the depth value
...
##########################################################################################################
        ...
        intrinsic_matrix = np.array([[714.5459, 0, 0], [0, 714.5459, 0], [0, 0, 1]])
        distortion_coeffs = np.array([0, 0, 0, 0, 0])
        distortion_coeffs = distortion_coeffs.reshape(1, 5)
        image_shape = (640, 480)

        # Assuming points_3d is the 3D point cloud obtained from SFM
        # points_3d should be a Nx3 numpy array, where N is the number of 3D points
        depth_map = generate_depth_map(total_points, intrinsic_matrix, distortion_coeffs, image_shape)

        # Save the depth map to a file or further process as needed
