...
/*保存视差点云为PLY文件*/
void SavePointCloudAsPLY(const uint8* img_bytes, const float32* disp_map, const sint32& width, const sint32& height, const std::string& path);
...
{                   
	...
	//···············································································//
	// 读取影像
	std::string path_left = "C:/Users/李在渊/Desktop/SaveFile/ppt/design/Multiview-3D-Reconstruction-main/Multiview-3D-Reconstruction-main/Datasets/myCV/0000.jpg";
	std::string path_right = "C:/Users/李在渊/Desktop/SaveFile/ppt/design/Multiview-3D-Reconstruction-main/Multiview-3D-Reconstruction-main/Datasets/myCV/0001.jpg";
	...
	// 保存点云
	SavePointCloudAsPLY(bytes_left, pms.GetDisparityMap(0), width, height, path_left);
	SavePointCloudAsPLY(bytes_right, pms.GetDisparityMap(1), width, height, path_left);
...
void SavePointCloudAsPLY(const uint8* img_bytes, const float32* disp_map, const sint32& width, const sint32& height, const std::string& path)
{
	...
	float32 B = 707.3739576785242;    // 基线
	float32 f = 719.5459;        // 焦距
	float32 x0l = 0;        // 左视图像主点x0
	float32 y0l = 0;        // 左视图像主点y0
	float32 x0r = 707.3739576785242; // 右视图像主点x0

	// 首先计算有效点的数量
	sint32 valid_points = 0;
	...
	FILE* fp_ply = nullptr;
	fopen_s(&fp_ply, (path + "2.ply").c_str(), "w");
	if (fp_ply) {
		// 写入PLY文件头
		fprintf_s(fp_ply, "ply\n");
		fprintf_s(fp_ply, "format ascii 1.0\n");
		fprintf_s(fp_ply, "element vertex %d\n", valid_points);
		fprintf_s(fp_ply, "property float x\n");
		fprintf_s(fp_ply, "property float y\n");
		fprintf_s(fp_ply, "property float z\n");
		fprintf_s(fp_ply, "property uchar red\n");
		fprintf_s(fp_ply, "property uchar green\n");
		fprintf_s(fp_ply, "property uchar blue\n");
		fprintf_s(fp_ply, "end_header\n");
